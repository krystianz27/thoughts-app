...
urlpatterns = [
    ...
    # 2 Show a success message stating that en email was sent to reset out password
    path(
        "reset_password_sent",
        auth_views.PasswordResetDoneView.as_view(
            template_name="journal/password-reset-sent.html"
        ),
    ...
    path(
        ...
        auth_views.PasswordResetConfirmView.as_view(
            template_name="journal/password-reset-form.html"
        ),
        name="password_reset_confirm",
    ),
    # 4 Show a success message stating that our password has been reset
    path(
        "password_reset_complete",
        auth_views.PasswordResetCompleteView.as_view(
            template_name="journal/password-reset-complete.html"
        ),
